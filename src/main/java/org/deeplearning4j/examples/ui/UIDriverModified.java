 ...
 */
public class UIDriverModified extends Application<NIDSConfig> {
    ...
    private static volatile UIDriverModified instance;

    private static final Logger log = LoggerFactory.getLogger(UIDriverModified.class);
    ...
    static {
        List<String> temp = Arrays.asList("-", "dns", "http", "smtp", "ftp-data", "ftp", "ssh", "pop3", "snmp", "ssl", "irc", "radius", "dhcp");
        List<String> temp2 = Arrays.asList("TRACE","DEBUG","INFO","WARN","ERROR","FATAL","Other","None","Custom0","Custom1","Custom2","Custom3","Custom4");
        serviceMap = new HashMap<>();
        for( int i=0; i<temp.size(); i++ ) serviceMap.put(temp.get(i),temp2.get(i));
    }

    private UIDriverModified(List<String> classNames, int normalClassIdx, List<String> serviceNames,
...
//            run("server", "dropwizard.yml");
    ...
    public void run(NIDSConfig configuration, Environment environment) {
        final UIResourceModified resource = new UIResourceModified();
    ...
    public static UIDriverModified getInstance() {
     ...
     * @return
     */
    public synchronized static UIDriverModified createInstance(List<String> classNames, int normalClassIdx, List<String> serviceNames,
                                                               TableConverter tableConverter, Map<String,Integer> columnsMap){
        ...
        instance = new UIDriverModified(classNames,normalClassIdx,serviceNames,tableConverter,columnsMap);
        return instance;
    }
        ...
        private void runHelper(){
            ...
            while(!shutdown.get()){
                ...
                for(Tuple3<Long,INDArray,List<Writable>> t3 : list){
                    ...
                    //Hacking thing for changing UI display...
                    if(serviceMap.containsKey(service)) service = serviceMap.get(service);
                    ...
                    if(probs[normalClassIdx] < 0.5f){
                        ...
                        RenderableComponent barChart = new RenderableComponentHorizontalBarChart.Builder()
                                ...
                                .title("Network Predictions: Error Probabilities")
...
//                        renderElementsList.add(new Pair<>(idx,re));
                ...
                String[][] table = new String[lastAttacks.size()][4];
                int j=0;
                for(Tuple3<Long,INDArray,List<Writable>> t3 : lastAttacks ){
                    ...
                    table[j][1] = l.get(columnsMap.get("source ip")).toString();
                    table[j][2] = attackProbStr;
                    table[j][3] = classNames.get(maxIdx);
                    j++;
                }

                RenderableComponentTable rct = new RenderableComponentTable.Builder()
                        .header("#","Machine IP","Error Probability","Type")
                        .table(table)
                        .paddingPx(5,5,0,0)
                        .border(1)
                        .backgroundColor("#FFFFFF")
                        .headerColor("#CCCCCC")
                        .colWidthsPercent(10,30,25,25)
                        .build();
