...
import org.deeplearning4j.examples.datasets.nb15.ui.NB15TableConverterModified;
import org.deeplearning4j.examples.streaming.CollectAtUIDriverFunction;
import org.deeplearning4j.examples.streaming.CollectAtUIDriverFunctionModified;
 ...
 *
 */
public class StreamingDemoEntryPoint2 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        List<String> labels = Arrays.asList("None", "Hardware (CPU)", "Hardware (Memory)", "Logged Stack Trace", "Service Stopped", "Network (Latency)",
                "Network (Bandwidth)", "Log Volume", "Other", "Unknown Error");
                //Arrays.asList("none", "Exploits", "Reconnaissance", "DoS",
//                "Generic", "Shellcode", "Fuzzers", "Worms", "Backdoor", "Analysis");    //NB15Util.LABELS;
//        List<String> services = NB15Util.SERVICES;
        List<String> services = Arrays.asList("TRACE","DEBUG","INFO","WARN","ERROR","FATAL","Other","None","Custom0","Custom1","Custom2","Custom3","Custom4");
                //Arrays.asList("-", "dns", "http", "smtp", "ftp-data", "ftp", "ssh", "pop3", "snmp", "ssl", "irc", "radius", "dhcp");
        int labelIdx = NB15Util.LABELIDX;
        int normalIdx = NB15Util.NORMALIDX;
        int nIn = NB15Util.NIN;
        int nOut = NB15Util.NOUT;
        TableConverter tableConverter = new NB15TableConverterModified(schema); //new NB15TableConverter(schema);
        ...
        UIDriverModified.createInstance(labels,normalIdx,services,tableConverter,columnMap);
...
//        sparkConf.setMaster("local[*]");
        ...
        predictionStream.foreachRDD(new CollectAtUIDriverFunctionModified());
